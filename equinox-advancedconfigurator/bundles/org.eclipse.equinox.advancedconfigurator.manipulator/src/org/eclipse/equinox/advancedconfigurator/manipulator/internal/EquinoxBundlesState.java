...
public class EquinoxBundlesState implements BundlesState {
	...
	public static final String[] PROPS = {
			"osgi.os", "osgi.ws", "osgi.nl", "osgi.arch", Constants.FRAMEWORK_SYSTEMPACKAGES, "osgi.resolverMode", Constants.FRAMEWORK_EXECUTIONENVIRONMENT, "osgi.resolveOptional", "osgi.genericAliases" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
	 ...
	 */
	private final HashMap locationStateIndex = new HashMap();
	 ...
	 */
	private final HashMap nameVersionStateIndex = new HashMap();
	 ...
	 */
	private boolean composeState(BundleInfo[] bInfos, Dictionary props, File fwPersistentDataLocation) throws IllegalArgumentException,
	...
	private void initialize(boolean useFwPersistentData) {
		...
		AdvancedEquinoxManipulatorImpl.checkConsistencyOfFwConfigLocAndFwPersistentDataLoc(launcherData);
