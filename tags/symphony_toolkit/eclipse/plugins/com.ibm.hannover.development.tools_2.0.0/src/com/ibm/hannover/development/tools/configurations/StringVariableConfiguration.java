...
public class StringVariableConfiguration implements IConfigure{
	...
	public static final String CLAZZ = StringVariableConfiguration.class.getName();
	...
	public void configure() {
		...
		try {
			...
			if(variables.size() > 0)
				try {
					stringVariableManager.addVariables((IValueVariable[]) variables.toArray(new IValueVariable[variables.size()]));
				} catch (CoreException e) {
					logger.logp(Level.SEVERE, CLAZZ, "updateVariable",
							"Fail to update variables.", e);
				}
		} catch (IOException e1) {
			logger.logp(Level.WARNING, CLAZZ, "configure", "Fail to load configuration.", e1);
		}
		...
		if(plugins.exists() && plugins.isDirectory()){
			...
			if(entries != null){
				try{
					...
					return rcpBase.substring(rcpBase.indexOf(RCP_BASE) + RCP_BASE.length());
				}catch(IOException e){
					e.printStackTrace();
				}
			}
