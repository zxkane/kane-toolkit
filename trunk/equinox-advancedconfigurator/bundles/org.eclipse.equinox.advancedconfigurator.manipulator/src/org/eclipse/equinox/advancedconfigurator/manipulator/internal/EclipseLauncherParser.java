...
public class EclipseLauncherParser {
	...
	void read(File launcherConfigFile, LauncherData launcherData) throws IOException {
		...
		if (osgiInstallArea == null) {
			osgiInstallArea = launcherData.getFwJar() != null ? ParserUtils.fromOSGiJarToOSGiInstallArea(launcherData.getFwJar().getAbsolutePath()).toURI()
					: launcherFolder;
		}
	...
	void save(EquinoxLauncherData launcherData, boolean backup) throws IOException {
		File launcherConfigFile = AdvancedEquinoxManipulatorImpl.getLauncherConfigLocation(launcherData);
