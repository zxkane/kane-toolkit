...
public class SymphonyPolicy {
	...
	public void configure() {
		...
		try {
			...
			Platform.getLog(Activator.getDefault().getBundle()).log(
					new Status(IStatus.ERROR, Activator.PLUGIN_ID,
							"Can't find installed symphony.", null));
			return;
		}
		...
		job.setTargetPlatform("Symphony"); //$NON-NLS-1$
		...
		if (!Platform.OS_MACOSX.equals(Platform.getOS())) {
			...
			if (select == 1) {
				...
				// job.setVMRootPath(FinderUtility.findDEEVMPath(installedPath));
				// // new code
				// Use J2SE vm since Symphony 2.0
				job
						.setVMProvider("com.ibm.hannover.development.tools.configurations.StardVMConfiguration"); //$NON-NLS-1$
				job.setVMRootPath(FinderUtility
						.findStandardVMPath(installedPath));
			}
		}
